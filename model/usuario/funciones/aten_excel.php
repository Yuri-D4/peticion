...
if (isset($_POST['aten_excel'])) { // Asegúrate de usar el nombre correcto de tu botón
    ...
    $result = $con->prepare("SELECT solicitudes.documento, usuarios.nombre, usuarios.apellido, usuarios.correo, tipo_solicitud.tipo_soli, solicitudes.fecha, solicitudes.descripcion, respuesta.respuesta, estado.nom_estado 
                            FROM solicitudes 
    ...
    foreach ($data as $item) {
        ...
        $sheet->setCellValue('H' . $row, $item['respuesta']);
        $sheet->setCellValue('I' . $row, $item['nom_estado']);
        $row++;
    }

    // Establecer encabezados para la descarga
    header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    header('Content-Disposition: attachment;filename="Solicitudes_Respondidas.xlsx"');
